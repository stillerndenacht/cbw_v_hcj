<?php $seitentitel = 'PDO Formular mit Security' ?>
<?php
...
if (isset($_POST['name']) && isset($_POST['vn']) && isset($_POST['textarea'])) {
    ...
    function serverout($name, $vn, $textarea, $meldung,$check,$db)
    {
        ...
        VALUES 
            ("' . $vn . '"," ' . $name . '"," ' . $textarea . '",' . time() . ');    
        ';
        echo $sqlin;
        if ($check){$db->exec($sqlin);}
...
<body>
    ...
    serverout($name, $vn, $textarea, $meldung,$check,$db);
