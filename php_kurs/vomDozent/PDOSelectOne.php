<?php     
	...
	if($id !== false && is_numeric($id)) {	# wenn nicht ID vorhanden und auch numerisch ist
		...
		/* --------------------------------------------
		# --------------------------------------------
		# Hier mit ? als Platzhalter für Prepare
		# --------------------------------------------
		#$sql = 'SELECT CONCAT(vorname, " ", nachname) AS fullname, info , zeit FROM Freunde WHERE id=?';
...
<html lang="de">
	...
	<body>
		...
		<?php if($isOk) { ?>
			...
			<p>
				<a href="PDODelete.php?id=<?= $id ?>">Freund löschen</a>
			</p>
